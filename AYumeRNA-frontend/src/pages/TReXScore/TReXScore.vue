<template>
  <div class="site--main">
    ...
    <div class="parameters-container">
      ...
      <p><strong>Reverse Codon:</strong> {{ generationParameters.reverseCodon }}</p>
      <p><strong>Sequence Count:</strong> {{ generationParameters.sequenceCount }}</p>
    </div>

    <button v-if="!taskSubmitted" @click="submitTask" class="submit-btn">Submit Task</button>
    <p v-else class="task-status">Task has been automatically submitted. Waiting for results...</p>
    ...
    <!-- 下载按钮 -->
    <button v-if="resultDownloadUrl" @click="downloadResult" class="download-btn">Download Results</button>
...
const loadLocalStorageData = () => {
  ...
  if (parameters) {
    try {
      generationParameters.value = JSON.parse(parameters) as GenerationParameters;
  ...
  if (sequenceData) {
    try {
      const parsedSequences = JSON.parse(sequenceData) as { sequence: string }[];
...
const submitTask = async () => {
  if (!aminoAcid.value || !domain.value || !generationParameters.value.reverseCodon) {
  ...
  const client = new Client({
    ...
    onConnect: () => {
      client.subscribe(subscribeUrl, async (message) => {
        ...
        const resultUrlMatch = body.match(/results uploaded:\s*(https?:\/\/\S+)/);
        if (resultUrlMatch) {
