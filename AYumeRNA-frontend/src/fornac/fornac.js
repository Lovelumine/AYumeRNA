...
function modification2SupOrSubForD3(text) {
  ...
  let reg2 = new RegExp(
    ',<tspan baseline-shift="super" style="font-size:0.2rem">[0-9]+</tspan>',
    'g',
  )
  let text1 = text
    .toString()
    .replace(
      reg1,
      '<tspan baseline-shift="super" style="font-size:0.2rem">$&</tspan>',
    )
...
export function FornaContainer(element, passedOptions = {}) {
  ...
  var vis = mouseEventHelper.append('svg:g').classed('fornac-plot', true)
  ...
  var xScale = d3.scale
    .linear()
    .domain([0, self.options.svgW])
    .range([0, self.options.svgW])
  var yScale = d3.scale
    .linear()
    .domain([0, self.options.svgH])
    .range([0, self.options.svgH])
  ...
  let toggleSelectNode = node => {
    ...
    visNodes
      ...
      .classed('fornac-selectedNode', function (p) {
        return p.selected
      })
  }
  ...
  let deselectAllNodes = () => {
    visNodes.selectAll('g.gnode').classed('fornac-selectedNode', function (p) {
  ...
  let createNewNodes = function (gnodesEnter) {
    ...
    var labelsEnter = gnodesEnter
      ...
      .html(function (d) {
        return modification2SupOrSubForD3(d.name)
      })
  ...
  function drawDirectionArrow(d) {
    ...
    let arrowTip = [
      (d.radius + nodeStroke) * u[0],
      (d.radius + nodeStroke) * u[1],
    ]
  ...
  self.transitionRNA = function (newStructure, nextFunction) {
    ...
    let gnodes = visNodes
      .selectAll('g.gnode')
      .data(newRNAJson.nodes, elementKey)
    var links = visLinks
      .selectAll('line.link')
    ...
    var newNodes = createNewNodes(gnodes.enter()).attr(
      'transform',
      function (d) {
        if (typeof d.x != 'undefined' && typeof d.y != 'undefined')
  ...
  let getBoundingBoxTransform = () => {
    ...
    var minRatio =
      Math.min(
        widthRatio,
        heightRatio,
        self.options.maxNodeRadius / maxRadius,
      ) * 0.8
  ...
  self.changeColorScheme = function (newColorScheme) {
    ...
    proteinNodes.classed('fornac-node', true).attr('r', function (d) {
      return d.radius
    })
  ...
  let updateStyle = () => {
    ...
    visNodes
      ...
      .classed('fornac-transparent', !self.displayParameters.displayNumbering)
    visNodes
    ...
    svg
      .selectAll('[node_type=nucleotide]')
      .classed('fornac-transparent', !self.displayParameters.displayNodeOutline)
    // Node Labels
    visNodes
      .selectAll('[label_type=nucleotide]')
      .classed('fornac-transparent', !self.displayParameters.displayNodeLabel)
    // Links
    svg
      .selectAll(
      ...
      .classed('fornac-transparent', !self.displayParameters.displayLinks)
    // Pseudoknot Links
    svg
      .selectAll('[link_type=pseudoknot]')
      .classed(
        'fornac-transparent',
        !self.displayParameters.displayPseudoknotLinks,
      )
    // Protein Links
    svg
      ...
      .classed(
        'fornac-transparent',
        !self.displayParameters.displayProteinLinks,
      )
    // Fake Links
    visLinks
      .selectAll('[link_type=fake]')
      .classed('fornac-transparent', !self.options.displayAllLinks)
    visLinks
      .selectAll('[link_type=fake_fake]')
      .classed('fornac-transparent', !self.options.displayAllLinks)
    // direction Arrows
    svg
      .selectAll('.' + 'fornac-directionArrow')
      .classed(
        'fornac-transparent',
        !self.displayParameters.displayDirectionArrows,
      )
  }
  ...
  let changeNode = function (nodeName, referenceNode) {
    ...
    let newSequence =
      sequence.slice(0, newNodeNum - 1) + nodeName + sequence.slice(newNodeNum)
  ...
  let insertNodeBeforeOrAfter = function (
    nodeName,
    referenceNode,
    positionOffset,
  ) {
    ...
    let newDotbracket =
      dotbracket.slice(0, newNodeNum) + '.' + dotbracket.slice(newNodeNum)
    let newSequence =
      sequence.slice(0, newNodeNum) + nodeName + sequence.slice(newNodeNum)
  ...
  let deleteNode = function (node) {
    ...
    let newDotbracket =
      dotbracket.slice(0, node.num - 1) + dotbracket.slice(node.num)
    let newPositions = positions
      .slice(0, node.num - 1)
  ...
  let removeBackBoneLink = function (d) {
    if (d.target.num - d.source.num != 1) {
      console.log(
        'ERROR: non adjacent nodes. Target:',
        d.target,
        'Source:',
        d.source,
        'Link:',
    ...
    if (index > -1) {
      ...
      if (d.source.rna == d.target.rna) {
        if (d.linkType == 'backbone') {
          console.log(
            'trying to remove a backbone link',
            d.source.num,
            d.target.num,
          )
  ...
  self.addNodes = function addNodes(json) {
    ...
    json.links.forEach(function (entry) {
      if (typeof entry.source == 'number')
        entry.source = json.nodes[entry.source]
      if (typeof entry.target == 'number')
        entry.target = json.nodes[entry.target]
    })
  ...
  self.addExternalLinks = function (rnaJson, externalLinks) {
    ...
    for (let i = 0; i < externalLinks.length; i++) {
      ...
      if (
        Object.prototype.toString.call(externalLinks[i][0]) === '[object Array]'
      ) {
      ...
      if (
        Object.prototype.toString.call(externalLinks[i][1]) === '[object Array]'
      ) {
        for (let j = 0; j < rnaJson.nodes.length; j++) {
          if ('nucs' in rnaJson.nodes[j]) {
      ...
      if (newLink.source == null || newLink.target == null) {
        console.log(
          'ERROR: source or target of new link not found:',
          newLink,
          externalLinks[i],
        )
        continue
