...
export function simpleXyCoordinates(pair_table) {
  ...
  len = pair_table[0]
  var angle = Array.apply(null, new Array(len + 5)).map(
     ...
     i-1 and j+1 are paired. */
  {
    ...
    var remember = Array.apply(
      null,
      new Array(3 + Math.floor((j - i) / 5) * 2),
    ...
    polygon =
      (Math.PI * (count - 2)) / count /* bending angle in loop polygon */
    remember[++r] = j
